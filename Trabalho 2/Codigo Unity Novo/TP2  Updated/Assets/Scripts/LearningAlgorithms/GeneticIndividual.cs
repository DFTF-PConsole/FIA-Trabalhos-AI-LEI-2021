...
public class GeneticIndividual : Individual {
   ...
   public override void Initialize(NeuralNetwork nn)
    ...
    public void MutateGaussian(float probability) {
        /* YOUR CODE HERE! */
        mean = Param.instance.mean;
        stdev = Param.instance.stdev;
        ...
        {
            if (Random.Range(0.0f, 1.0f) < probability)
                this.genotype[i] += NextGaussian(mean, stdev);
        }
    }
    ...
    public override void Crossover(Individual partner, float probability)
    {
        ...
        {
            float aux;
            for (int x = Random.Range(0, this.genotype.Length - 1); x < this.genotype.Length; x++)
            {
                aux = partner[x];
                partner[x] = this[x];
                this[x] = aux;
